...
export const Upravlenie: React.FC<Props> = ({ onChange, vodilaChange }) => {
  ...
  // const [upravlenie, setUpravlenie] = useState('Авто')
  // const [oldupravlenie, setOldupravlenie] = useState('-')
  ...
  // const onUprClick = useCallback((txt: any) => {
  //   setOldupravlenie(txt)
  // }, [])
  ...
  useEffect(() => {
    setElems(spisok.map((item, index) => {
      if (numCat.find(i => item.category === i)) {
        return (<View key={index} style={[styles.whiteUpravlenie, { marginVertical: 10, justifyContent: 'center' }, styles.vnutriKnopki]}>
          <Pressable
            key={index}
            style={{ flexDirection: 'row' }}
            onPress={() => { setModalVisible(!modalVisible), setVodila(item) }}>
            <Text style={styles.wt}>{item.name}</Text>
            <Image source={cat[item.category]} style={styles.img} contentFit="cover" />
          </Pressable>
          <Pressable
            onPress={() => waPress(item.number, item.name)}
          >
            <Image source={require('../../assets/wa.png')} style={styles.img} contentFit="cover" />
          </Pressable>
          <Pressable
            onPress={() => tel(item.number)}
          >
